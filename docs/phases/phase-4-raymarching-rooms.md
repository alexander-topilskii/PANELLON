...
- [ ] Асинхронное чтение пикселей через WebGL2 PBO (`PIXEL_PACK_BUFFER` и `gl.fenceSync`) без блокировки главного потока
- [ ] Оценка средней яркости и variance после получения данных от GPU

### 4.6b Retry-логика

- [ ] Чёрный экран: яркость < 0.02 AND variance < 0.001
- [ ] При провале: Room_ID + salt, до **3-5 попыток** (чтобы не допускать фризов)
- [ ] Проверки (генерация, компиляция, валидация) распределяются во времени (time-slicing): не более 1-2 попыток валидации за кадр. Пока комната валидируется, проход недоступен.
- [ ] При полном провале (после 5 попыток): ячейка → `"empty"` (глухая стена, нет двери)
